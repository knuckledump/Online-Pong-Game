...
class game:
    ...
    def send_data(self):
        self.socket.send(str(g.right_player.rect.y).encode("utf-8"))

    def recv_data(self):
        msg = self.socket.recv(1024).decode('utf-8')
        data = msg.split('/')
        self.left_player.rect.y = int(data[0])
        self.ball.rect.x = int(data[1])
        self.ball.rect.y = int(data[2])
